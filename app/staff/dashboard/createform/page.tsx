...
export default function Createformpage() {
  ...
  // set up form/empty form
  const [isFormInitial, setFormInitial] = useState<FormInnitType>();
  const [isQuestions, setQuestions] = useState<CreateQuestionType>();
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 w-[95%] sm:w-[85%] md:w-[65%] lg:w-[55%] 2xl:w-[50%] mb-2">
        ...
        <div className="mt-5">
          ...
          <div className=" rounded-t-md rounded-b-sm  h-auto  flex flex-col gap-4 w-full items-center ">
            {form.questions.map((q) => (
              <div
                key={q.id}
                className=" w-full flex justify-center items-center flex-col gap-2 px-5 md:px-10 pb-10 pt-2 mt-5 rounded-xl bg-[#f5f5f5] dark:bg-woodsmoke-950 shadow-sm  "
              >
                ...
                {q.type == "textarea" && (
                  <div className="w-full">
                    <textarea
                      placeholder="answer here!"
                      value={q.title}
                      className="borderlessInput transition-all duration-75 ease-in-out px-2 w-full min-h-[100px]  bg-woodsmoke-100 text-2xl rounded-md font-medium"
                    />
                  </div>
                )}
                    ...
                    <RadioGroup>
                      <div className="flex items-center gap-3 w-full justify-start">
                        ...
                        <input
                          ...
                          className="borderlessInput bg-havelock-300 border-b-2 border-b-havelock-blue-800  transition-all duration-75 ease-in-out px-2 w-full h-auto py-2 bg-havelock-blue-100  text-md rounded-t-md "
                        />
                        <button
        ...
        <div className="flex justify-center items-center p-4 rounded-sm">
          <button
            className="h-14 w-80 flex justify-center items-center gap-3 text-black dark:text-white bg-[#eeeeee] dark:bg-woodsmoke-600 rounded-xl cursor-pointer shadow-sm shadow-woodsmoke-400"
            onClick={() => {
              // dispatch({
              //   type: "SET_FORMID",
              //   formId: uuidv4(),
        ...
        <div className="flex justify-between items-center p-4  rounded-sm">
          ...
          {form.questions.length > 0 && (
            <button
              ...
              className="h-14 w-40 flex justify-center items-center  text-white bg-amber-600 rounded-sm cursor-pointer"
            >
              Delete
            </button>
          )}
      ...
      <div className="mt-20 p-10 w-full flex justify-center items-center h-16 bg-havelock-blue-600 text-white">
